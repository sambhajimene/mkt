...
# ================= SCANNER FUNCTION =================
def scan_bullish():
    """Scan NSE500 Cash Stocks for Bullish HA setup"""
    ...
    for _, row in df_nse500.iterrows():
        ...
        try:
            ...
            strong_daily = (
                last_daily['HA_Close'] > last_daily['HA_Open'] and
                last_daily["close"] > last_daily["EMA50"]
            )

            daily_condition = neutral_daily or strong_daily
            ...
            if hourly_strong:
                results.append({"Symbol": symbol, "CMP": last_daily["close"]})
...
def scan_bearish():
    """Scan NSE Futures for Bearish HA setup"""
    instruments = kite.instruments("NFO")
    df_instruments = pd.DataFrame(instruments)
    df_fut = df_instruments[df_instruments["instrument_type"] == "FUT"]
    ...
    for _, row in df_fut.iterrows():
        ...
        try:
            ...
            weekly_body = last_weekly['HA_Close'] - last_weekly['HA_Open']
            weekly_upper_wick = last_weekly['HA_High'] - max(last_weekly['HA_Close'], last_weekly['HA_Open'])
            ...
            hourly_body = last_hourly['HA_Close'] - last_hourly['HA_Open']
            hourly_upper_wick = last_hourly['HA_High'] - max(last_hourly['HA_Close'], last_hourly['HA_Open'])
            hourly_range = last_hourly['HA_High'] - last_hourly['HA_Low']
...
# ================= SAVE SIGNALS =================
def save_signals(signals_dict):
    with open(SIGNAL_STORE_FILE, "w") as f:
        json.dump(signals_dict, f, indent=2)

# ================= STREAMLIT UI =================
st.set_page_config(layout="wide")
st.title(" BuBe HA Scan")
...
if run_scan or auto_mode:
    with st.spinner("Scanning NSE500 Cash & NFO Futures..."):
        bullish_signals = scan_bullish()
        bearish_signals = scan_bearish()

        st.subheader("📈 Bullish Signals (Cash Stocks)")
        if bullish_signals:
        ...
        st.subheader("📉 Bearish Signals (Futures)")
        if bearish_signals:
        ...
        save_signals({"bullish": [s['Symbol'] for s in bullish_signals],
                      "bearish": [s['Symbol'] for s in bearish_signals]})
