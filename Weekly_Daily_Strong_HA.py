...
def is_strong_ha(df):
    return abs(df['HA_Open'].iloc[-1] - df['HA_Low'].iloc[-1]) < 0.01 and df['HA
_Close'].iloc[-1] > df['HA_Open'].iloc[-1]
...
for stock in nse500:
    try:
        ...
        # Scanner Logic: Weekly + Daily strong HA
        if weekly_strong and daily_strong:
...
print("\n===== WEEKLY + DAILY STRONG HA STOCKS =====")
for r in results:
    print(r)

if not results:
    print("No stocks matched this week + day.")
