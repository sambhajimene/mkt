...
if not access_token:
    st.warning("Enter Fyers token to continue")
    st.stop()
...
refresh_minutes = st.sidebar.number_input("Refresh interval (minutes)", 1, 30, REFRESH_MINUTES)
...
if oc:
    ...
    if show_loc:
        def highlight_atm(s):
            return ['background-color: yellow' if atm-100 <= v <= atm+100 else '' for v in s]
    ...
    for strike in oc:
        ...
        if bias != "NO TRADE":
            ...
            if score >= 60 and should_alert(symbol, strike["strike"], bias):
                ...
                send_email(f"High-Confidence Alert: {symbol}", f"{symbol} {bias} Strike {strike['strike']} Confidence {score}%")
